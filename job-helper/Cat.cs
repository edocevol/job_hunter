...
namespace 自动截屏并上传
{
    public partial class Cat : Form
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                e.Graphics.DrawRectangle(new Pen(Color.HotPink, 4), recttangle);//绘制矩形
        ...
        private void FormMove()
        {
            ...
            if (sr.Y - b.Y > 40)
            {
                pointShow.Location = new Point(b.X-185, b.Y);
            }
            else
            {
                if (a.Y > 40)
                {
                    pointShow.Location = new Point(b.X - 185, a.Y - 30);
                }
                else
                {
                    pointShow.Location = new Point(b.X - 185, b.Y - 30);
        ...
        public void Save()
        {
            try
            {
                ...
                loading.Show();
                var result = (new SouGouOCR()).SogouOCR(image);
                (new Result(result, loading)).Show();
                pointShow.Close();
                this.Close();
            }
            catch (Exception ex)
